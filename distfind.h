...
class GaussKeelinMixtureEvolutionPDF : public SubspaceState {

private:

    SumConstraint constraintAmplitudes;
    SumConstraint constraintAmplitudes0;
...
public:
    ...
    GaussKeelinMixtureEvolutionPDF(const ProbabilityDistributionSamples& data, size_t nModes, int nTerms) :
        SubspaceState({"A", "A0", "mu", "mu0", "sig", "sig0", "a", "a0", "pdfX", "pdfY", "cdf", "pdfX0", "pdfY0", "cdf0", "m1", "nNonzeroModes"}, 8, false), constraintAmplitudes(1), constraintAmplitudes0(1), nModes(nModes), nTerms(nTerms), data(&data) {
            ...
            setCoords({std::vector<Float>(nModes + 1, Float(1)/(nModes+1)),
                        std::vector<Float>(nModes + 1, Float(1)/(nModes+1)),
                        std::vector<Float>(nModes, 0),
                        std::vector<Float>(nModes, 0),
                         std::vector<Float>(nModes, std/std::max(size_t(5), nModes)),
                         std::vector<Float>(nModes, std/std::max(size_t(5), nModes)),
            ...
            for (size_t i = 0; i < nModes; ++i) {
                coords[2][i] = mean-3*std + (i+0.5)*(6*std)/(nModes);
                coords[3][i] = mean-3*std + (i+0.5)*(6*std)/(nModes);
            }

            constraintAmplitudes.link(&coords[0]);
            constraintAmplitudes0.link(&coords[1]);
    ...
    std::vector<Float> getInitialConditions() override {
        ...
        for (size_t i = 0; i < nModes; ++i)
            ret.push_back(coords[4][i]);
        for (size_t i = 0; i < nModes; ++i)
            ret.push_back(coords[5][i]);

        for (size_t i = 0; i < 10; ++i)
            ret.push_back(coords[6][i]);
        for (size_t i = 0; i < 10; ++i)
            ret.push_back(coords[7][i]);
    ...
    void setInitialConditions(const std::vector<Float>& ics) override {
        ...
        for (size_t i = 0; i < nModes; ++i)
            coords[2][i] = ics[2*nModes+2+i];
        for (size_t i = 0; i < nModes; ++i)
            coords[3][i] = ics[3*nModes+2+i];

        for (size_t i = 0; i < nModes; ++i)
            coords[4][i] = ics[4*nModes+2+i];
        for (size_t i = 0; i < nModes; ++i)
            coords[5][i] = ics[5*nModes+2+i];

        for (size_t i = 0; i < 10; ++i)
            coords[6][i] = ics[6*nModes+2+i];
        for (size_t i = 0; i < 10; ++i)
            coords[7][i] = ics[6*nModes+12+i];
    }
    ...
    void compute_derived_late(const SharedParams& shared) override {
        ...
        for (int i = 0; i < pdfRes; ++i) {
            coords[8][i] = keelin_Q(y, coords[6]);
            coords[9][i] = coords[0][nModes]*keelin_pdf_of_y(y, coords[6]);
            coords[10][i] = coords[0][nModes]*y;
            coords[11][i] = keelin_Q(y, coords[7]);
            coords[12][i] = coords[1][nModes]*keelin_pdf_of_y(y, coords[7]);
            coords[13][i] = coords[1][nModes]*y;
            for (int m = 0; m < nModes; ++m) {
                Float arg = coords[8][i] - coords[2][m];
                Float var = coords[4][m]*coords[4][m];
                coords[9][i] += coords[0][m]/(enoki::sqrt(2*pi*var))*enoki::exp(-arg*arg/(2*var));
                coords[10][i] += coords[0][m]*Float(0.5)*(1+enoki::erf(arg/(std::sqrt(Float(2.0))*coords[4][m])));

                arg = coords[11][i] - coords[3][m];
                var = coords[5][m]*coords[5][m];
                coords[12][i] += coords[1][m]/(enoki::sqrt(2*pi*var))*enoki::exp(-arg*arg/(2*var));
                coords[13][i] += coords[1][m]*Float(0.5)*(1+enoki::erf(arg/(std::sqrt(Float(2.0))*coords[5][m])));
            }
            y += dy;
        }
    ...
    void eval(const SharedParams& shared) override {
        ...
        for (size_t i = 0; i < enoki::packets(data->y); ++i) {
            ...
            FloatP p_i(0);
            FloatP x   = enoki::packet(data->y, i);
            FloatP t   = enoki::packet(data->sig, i);

            for (size_t m = 0; m < nModes; ++m) {
                /* do not support sample error now */
                // FloatP var = enoki::packet(data->sig, i);
                FloatP var{0};
                FloatP arg = x - t*coords[2][m] - (1-t)*coords[3][m];
                // var *= var;
                var += t*coords[4][m]*coords[4][m] + (1-t)*coords[5][m]*coords[5][m];
                p_i += (t*coords[0][m]+(1-t)*coords[1][m])*(enoki::sqrt(2*pi*var))*enoki::exp(-arg*arg/(2*var));
            }

            std::vector<FloatP> a{coords[6][0], coords[6][1], coords[6][2], coords[6][3], coords[6][4], coords[6][5], coords[6][6], coords[6][7], coords[6][8], coords[6][9]};
            for (size_t j = 0; j<nTerms; ++j) {
                a[j] *= t;
                a[j] += (1-t)*coords[7][j];
            }

            FloatP p_i_keelin = (t*coords[0][nModes]+(1-t)*coords[1][nModes])*keelin_pdf(x, a);
        ...
        m2 *= coords[0][nModes];
        for (size_t m = 0; m < nModes; ++m) {
            m2 += coords[0][m]*( (coords[2][m]-m1)*(coords[2][m]-m1) + coords[4][m]*coords[4][m] );
        }
...
(31*a6*a6*a6*a6*a6*a6*pi6)/1.118208e6;
        ...
        m1 = coords[1][nModes]*(a1 + a3/2. + a5/12. + a8/12.);
        for (size_t m = 0; m < nModes; ++m) {
            m1 += coords[1][m]*coords[3][m];
        }

        m2 = a1*a1 + a1*a3 + a3*a3/3. + a2*a4 + a4*a4/12. + (a1*a5)/6. + (a3*a5)/6. + a5*a5/80. + (2*a2*a6)/3. + (a4*a6)/6. + a6*a6/12. - \
                    2*a1*m1 - a3*m1 - (a5*m1)/6. + m1*m1 + (a2*a2*pi2)/3. + (a3*a3*pi2)/36. + (a2*a6*pi2)/18. + (a6*a6*pi2)/240.;

        m2 *= coords[1][nModes];
        for (size_t m = 0; m < nModes; ++m) {
            m2 += coords[1][m]*( (coords[3][m]-m1)*(coords[3][m]-m1) + coords[5][m]*coords[5][m] );
        }

        m3 = (a2*a3*a4)/2. + (a3*a4*a4)/8. + a2*a2*a5 + (a3*a3*a5)/24. + \
(a2*a4*a5)/4. + (a4*a4*a5)/60. + (a3*a5*a5)/120. + a5*a5*a5/3780. + \
(a2*a3*a6)/2. + (a3*a4*a6)/4. + (a2*a5*a6)/3. + (13*a4*a5*a6)/240. + \
(a3*a6*a6)/8. + (3*a5*a6*a6)/80. + a2*a2*a3*pi2 + (a3*a3*a3*pi2)/24. \
+ (a2*a3*a4*pi2)/6. + (a3*a3*a5*pi2)/180. + (5*a2*a3*a6*pi2)/12. + \
...
(a5*a6*a6*pi2)/840.;
    ...
    m4 = a3*a3*a3*a3/80. + (a2*a3*a3*a4)/2. + 2*a2*a2*a4*a4 + \
(a3*a3*a4*a4)/8. + (a2*a4*a4*a4)/3. + a4*a4*a4*a4/80. + a2*a2*a3*a5 + \
(a3*a3*a3*a5)/24. + (5*a2*a3*a4*a5)/6. + (3*a3*a4*a4*a5)/40. + \
(a2*a2*a5*a5)/6. + (a3*a3*a5*a5)/45. + (a2*a4*a5*a5)/15. + \
(11*a4*a4*a5*a5)/2520. + (a3*a5*a5*a5)/420. + a5*a5*a5*a5/15120. + \
...
(7*a6*a6*a6*a6*pi4)/34560.;
    ...
    m5 = (a2*a3*a3*a3*a4)/4. + (5*a2*a2*a3*a4*a4)/2. + (5*a3*a3*a3*a4*a4)/48. \
+ (5*a2*a3*a4*a4*a4)/6. + (7*a3*a4*a4*a4*a4)/144. + a2*a2*a3*a3*a5 + \
(a3*a3*a3*a3*a5)/48. + 5*a2*a2*a2*a4*a5 + (25*a2*a3*a3*a4*a5)/24. + \
(5*a2*a2*a4*a4*a5)/3. + (7*a3*a3*a4*a4*a5)/48. + \
(13*a2*a4*a4*a4*a5)/72. + (a4*a4*a4*a4*a5)/168. + \
...
(11*a3*a6*a6*a6*a6*pi4)/1440. + (7*a5*a6*a6*a6*a6*pi4)/57024.;
    ...
    m6 = a3*a3*a3*a3*a3*a3/448. + (3*a2*a3*a3*a3*a3*a4)/16. + \
3*a2*a2*a3*a3*a4*a4 + (5*a3*a3*a3*a3*a4*a4)/64. + 5*a2*a2*a2*a4*a4*a4 \
+ (5*a2*a3*a3*a4*a4*a4)/4. + (5*a2*a2*a4*a4*a4*a4)/4. + \
(19*a3*a3*a4*a4*a4*a4)/192. + (23*a2*a4*a4*a4*a4*a4)/240. + \
a4*a4*a4*a4*a4*a4/448. + (3*a2*a2*a3*a3*a3*a5)/4. + \
...
(31*a6*a6*a6*a6*a6*a6*pi6)/1.118208e6;
        ...
        w = var/N;
        // std::cout << w << "\t";
        t1 = 4*Float(0.5)*(m1-mean)*(m1-mean)/w;

        /* variance of empirical variance - estimated as var^2 / N */
        w *= var;
        t2 = 4*Float(0.5)*(m2-var)*(m2-var)/w;

        w *= var*(2*2);
        t3 = 6*Float(0.5)*m3*m3/w;

        w *= var*(3*3);
        k4 = m4 - 3*m2*m2;
        t4 = 8*Float(0.5)*k4*k4/w;

        w *= var*(4*4);
        k5 = m5 - 10*m3*m2;
        t5 = 12*Float(0.5)*k5*k5/w;

        w *= var*(5*5);
        k6 = m6 - 15*m4*m2 - 10*m3*m3 + 30*m2*m2*m2;
        t6 = 12*Float(0.5)*k6*k6/w;
    ...
    Proposal step(pcg32& rnd, const SharedParams& shared) const override {
        ...
        auto newstate = std::dynamic_pointer_cast<GaussKeelinMixtureEvolutionPDF>(copy());
        ...
        for (int k = 0; k < nSteps; ++k) {
            ...
            if (stepkind < ampstepthresh) {
                ...
                Float val;
                Float propR = 0;
                newstate->constraintAmplitudes.step(rnd, std::min(2*stepsizeCorrectionFac/nModes/std::sqrt(N), Float(2.0)/nModes), from, to, val, propR);
                propRatio *= propR;
                newstate->constraintAmplitudes0.step(rnd, std::min(2*stepsizeCorrectionFac/nModes/std::sqrt(N), Float(2.0)/nModes), from, to, val, propR);
                propRatio *= propR;
            }
            else {
                //
                ...
                //for (size_t i = 0; i < 1; ++i) {
                    ...
                    if (stepkind < otherthresh) {
                        ...
                        if (rnd.nextFloat() < 0.3f) {
                            newstate->coords[2][whichMode] += (rnd.nextFloat()-0.5)*8*std*std::min(stepsizeCorrectionFac, Float(1));
                            newstate->coords[3][whichMode] += (rnd.nextFloat()-0.5)*8*std*std::min(stepsizeCorrectionFac, Float(1));
                        }
                        else {
                            newstate->coords[2][whichMode] += 8*(rnd.nextFloat()-0.5)/std::sqrt(N)*std*std::min(stepsizeCorrectionFac, Float(1));
                            newstate->coords[3][whichMode] += 8*(rnd.nextFloat()-0.5)/std::sqrt(N)*std*std::min(stepsizeCorrectionFac, Float(1));
                        }


                        bound(newstate->coords[2][whichMode], mean-4*std, mean+4*std);
                        bound(newstate->coords[3][whichMode], mean-4*std, mean+4*std);
                         ...
                         * it is important to be able to have a shortcut to small sigma so amplitude can raise again in that case */
                        if (rnd.nextFloat() < 0.3f) {
                            newstate->coords[4][whichMode] += (rnd.nextFloat()-0.5)*(maxSigma-minSigma)*std::min(stepsizeCorrectionFac, Float(1));
                            newstate->coords[5][whichMode] += (rnd.nextFloat()-0.5)*(maxSigma-minSigma)*std::min(stepsizeCorrectionFac, Float(1));
                        }
                        else {
                            newstate->coords[4][whichMode] += (rnd.nextFloat()-0.5)*(maxSigma-minSigma)/std::sqrt(N)*std::min(stepsizeCorrectionFac, Float(1));
                            newstate->coords[5][whichMode] += (rnd.nextFloat()-0.5)*(maxSigma-minSigma)/std::sqrt(N)*std::min(stepsizeCorrectionFac, Float(1));
                        }

                        bound(newstate->coords[4][whichMode], minSigma, maxSigma);
                        bound(newstate->coords[5][whichMode], minSigma, maxSigma);
                    }
                }
                else {
                    int idx = std::min(int(rnd.nextFloat()*nTerms), nTerms-1);
                    ...
                    newstate->coords[6][idx] += (rnd.nextFloat()-0.5)*4*std/std::sqrt(N)*std::min(stepsizeCorrectionFac, Float(1));
                    newstate->coords[7][idx] += (rnd.nextFloat()-0.5)*4*std/std::sqrt(N)*std::min(stepsizeCorrectionFac, Float(1));
                    }
                }
        }
    ...
    std::shared_ptr<SubspaceState> copy() const override {
        auto foo = new GaussKeelinMixtureEvolutionPDF(*this);
        auto& f = foo->getCoordsAt("A");
        foo->constraintAmplitudes.link(&f);
        auto& f0 = foo->getCoordsAt("A0");
        foo->constraintAmplitudes0.link(&f0);
        //foo->constraint.link(&(foo->getCoords()[0]));
        return std::shared_ptr<SubspaceState>(foo);
    }
private:
